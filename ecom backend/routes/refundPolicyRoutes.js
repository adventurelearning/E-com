const refundPolicySchema=require('../models/RefundPolicy');
const express=require('express');
const router=express.Router();

router.get('/',async(req,res)=>{
  try {
    const policies=await refundPolicySchema.find();
    res.json(policies);
  } catch (error) {
...
router.post('/',async(req,res)=>{
  try {
    const newPolicy=new refundPolicySchema(req.body);
...
router.put('/:id',async(req,res)=>{
  try {
    const { id } = req.params;
    const updatedPolicy = await refundPolicySchema.findByIdAndUpdate(id, req.body, { new: true });
    if (!updatedPolicy) {
