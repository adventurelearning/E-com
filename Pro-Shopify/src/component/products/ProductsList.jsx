...
const ProductCard = ({ product }) => {
  ...
  return (
    <motion.div
      className="border rounded-lg p-3 sm:p-4 hover:shadow-lg transition-shadow bg-white flex flex-col h-full cursor-pointer"
            ...
            <div className="mt-2 text-[11px] sm:text-xs text-green-700">
              ...
              {product.offers.length > 1 && (
                <button
                  ...
                  className="text-blue-600 flex items-center mt-1 text-xs sm:text-sm"
                >
                  {showAllOffers ? (
                    <>
                      <span>Show less</span>
...
const ProductList = () => {
  ...
  const [visibleCount, setVisibleCount] = useState(8); // Initial count per category
  const navigate = useNavigate();
  ...
  const filteredProducts = () => {
    ...
    if (filter === 'discount') {
      result = result.filter(p => {
        const discount = ((p.originalPrice - (p.discountPrice || p.originalPrice)) / p.originalPrice) * 100;
        return discount > 20;
      });
    } else if (filter === 'new') {
  ...
  const handleLoadMore = () => {
    setVisibleCount(prev => prev + 8); // Load 8 more products per category
  };
            ...
            <div key={category}>
              ...
              <div className="sm:hidden overflow-x-auto">
                <div className="flex space-x-3 min-w-max">
                  {categoryProducts.map((product) => (
                    <div key={product._id} className="w-1/2 flex-shrink-0">
