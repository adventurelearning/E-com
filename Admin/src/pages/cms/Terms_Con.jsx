...
const Terms_Con = () => {
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        const response = await Api.get('/Terms_Con');
        if (response.data) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (policyId) {
        ...
        await Api.put(`/Terms_Con/${policyId}`, {
          title,
          description: des,
        });
      } else {
        // Create new policy
        const res = await Api.post('/Terms_Con', {
          title,
          description: des,
        });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-md mt-10">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Terms and Conditions</h1>
        ...
        <div className="flex items-center space-x-4">
          ...
          {success && (
            <p className="text-green-600 text-sm">
              Terms and conditions {policyId ? 'updated' : 'saved'} successfully!
            </p>
          )}
        </div>
      </form>
