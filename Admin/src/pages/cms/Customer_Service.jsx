...
const Customer_Service = () => {
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        const response = await Api.get('/Customer_Service');
        if (response.data) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (policyId) {
        ...
        await Api.put(`/Customer_Service/${policyId}`, {
          title,
          description: des,
        });
      } else {
        // Create new policy
        const res = await Api.post('/Customer_Service', {
          title,
          description: des,
        });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-md mt-10">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Customer Service</h1>
        ...
        <div className="flex items-center space-x-4">
          ...
          {success && (
            <p className="text-green-600 text-sm">
              Customer Service {policyId ? 'updated' : 'saved'} successfully!
            </p>
          )}
        </div>
      </form>
